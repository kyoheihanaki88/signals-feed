...
**Editorial-quality gates.** The Ranker keeps non-core content out of the top five: shopping **deals** (price/`$ off`/`% off`/Prime Day/buying guides), **personal first-person essays** ("My yard…, so I made an app"), and product **reviews** are skipped (logged as "skipped … for editorial quality"), appearing only if there is truly nothing better. It also nudges up civic/geopolitical/economic/institutional headlines. `writer.py validate --strict` additionally hard-fails drafts whose **summary** is too short, begins mid-sentence, is a headline fragment, or is cut off, and whose **whyItMatters** is a bare quote, a fragment, or copied verbatim from the summary — catching broken extraction before publish. (Deal/essay matching is headline-only and price-cue-based, so "a nuclear/trade deal" is never mistaken for a shopping deal.)
