...
test("A. every Japan alias collapses to one identity", () => {
  ...
  assert.equal(
    [...identities][0],
    "date=2026-07-27|regions=japan|topics=tech|selector=3|size=5",
  );
});
...
test("B3. the identity string has the exact Python shape", () => {
  assert.equal(
    ...
    "date=2026-07-27|regions=japan,united_states|topics=business,tech|selector=3|size=5",
  );
  // Empty topics still emit the key with an empty value.
  assert.equal(
    mixIdentity("2026-07-27", ["japan"], []),
    "date=2026-07-27|regions=japan|topics=|selector=3|size=5",
  );
});
