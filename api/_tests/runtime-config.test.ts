...
test("a valid Production environment produces a Production runtime config", () => {
  ...
  assert.equal(config.customMix.selectorVersion, 3);
...
test("H. a selector version other than the supported one (3) fails", () => {
  for (const value of ["1", "2", "0", "v1"]) {
    assert.ok(
      issuesOf(() =>
